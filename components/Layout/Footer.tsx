...
export default function Footer({ darkMode }: { darkMode?: boolean }) {
  // WhatsApp API link with custom pre-filled message
  // Using api.whatsapp.com format for better compatibility
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
        <div className={`flex flex-col sm:flex-row items-center justify-center gap-2 text-sm ${darkMode ? 'text-gray-300' : 'text-gray-600'}`}>
          ...
          <a
            ...
            className={`font-semibold ${darkMode ? 'text-gold hover:text-white' : 'text-navy hover:text-gold'} transition-colors underline flex items-center gap-1`}
          >
            <svg 
              className="w-4 h-4" 
              fill="currentColor" 
